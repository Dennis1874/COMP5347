<template>
  <div >
    <el-row>
      <el-select v-model="article" :remote-method="articleRemoteMethod" style="width:250px" filterable remote placeholder="please enter article name">
        <el-option v-for="(item,index) in selectArticle.options" :key="index" :label="item" :value="item"></el-option>
      </el-select>
      <el-button type="primary" @click="articleQuery()" :loading="queryloading">Qury</el-button>
  ...
  methods: {
    ...
    articleQuery() {
      ...
      if (this.article !== null && this.article != "") {
        overallApi
          .queryIfnewArticles({ name: this.article })
          .then(res => {
            this.queryloading = false;
            if (res.data.num == 1) {
              this.$message.warning("no new revisions");
            } else {
              this.$message.success(
                "success! " + res.data.num-1 + " revisions were fetched"
              );
            }
          ...
          .catch(error => {
            this.queryloading = false;
            console.log("query if new article:" + error);
          });
      }
    },
    getStatistics() {
